...
def _join_human(labels: List[str], T: dict) -> str:
    ...
    if len(out) == 1:
        return out[0]
    if len(out) == 2:
        return f"{out[0]} {T['and_word']} {out[1]}"
    return f"{T['comma'].join(out[:-1])} {T['and_word']} {out[-1]}"

def _roles_label(role_codes: List[str], T: dict) -> str:
    codes = [c.strip().lower() for c in (role_codes or []) if c and str(c).strip()]
    labels = [T["role_labels"].get(c, c) for c in codes]
    roles = _join_human(labels, T)
...
def about_contributor_block(
    ...
    T = _t(language_code)
    roles = _roles_label(contributor_role_codes, T)
    title = T["about_contributor_title"].format(roles=roles)
    body = (custom_text or T["about_contributor_default"]).format(name=contributor_name)
    return f"{title}\n\n{body}".strip() + "\n"
...
def about_edition_block(
    ...
    T = _t(language_code)
    title = T["about_edition_title"]
    body = (custom_text or T["about_edition_default"])
    return f"{title}\n\n{body}".strip() + "\n"
