...
from gaiden.units_db import get_units_for_chunking
...
def run_chunk_01(book_id: int, min_tokens: int = 1500, target_tokens: int = 1800, max_tokens: int = 2200) -> int:
    # Require split_struct
    units = get_units_for_chunking(book_id)
    if not units:
        raise ValueError("Split required: run SPLIT (structure) first.")
    ...
    # NOTE: chunker currently re-detects units internally; we keep it simple for now.
    # Since you want strict separation, the gate here enforces that SPLIT must exist.
    stage = "split_01"
    method = "structure_token_chunks_v1"
