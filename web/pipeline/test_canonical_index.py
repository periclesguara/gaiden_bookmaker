...
class CanonicalIndexFlowTests(TestCase):
    ...
    def test_freeze_pretruth_uses_fixed(self):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            fixed.write_text("# Chapter 01 - Intro\nBody\n", encoding="utf-8")
            # Keep normalized present to prove fixed has precedence.
            (fixed.parent / "normalized.md").write_text("# Chapter 01 - Raw\n", encoding="utf-8")
            ...
            self.assertEqual(result["truth_path"], edition.truth_path)
            self.assertEqual(len(edition.truth_sha256), 64)
            self.assertIn("_pretruth_freeze_", result["canonical_run_dir"])
            self.assertTrue((root / result["canonical_run_dir"] / "SHA256SUMS.txt").exists())
            self.assertTrue((root / result["canonical_run_dir"] / "manifest.json").exists())
    ...
    def test_freeze_pretruth_falls_back_to_normalized(self):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            normalized.write_text("# Chapter 01 - Intro\nBody\n", encoding="utf-8")
