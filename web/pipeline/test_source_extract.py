...
from pipeline.test_support import IsolatedStorageSimpleTestCase
...
class SourceExtractTests(IsolatedStorageSimpleTestCase):
    patch_storage_repo_root = True

    def setUp(self):
        self.source_fixture_root = self.test_fixture_root / "source_extract"
        self.source_fixture_root.mkdir(parents=True, exist_ok=True)

    def test_txt_extract_generates_canonical_txt_and_html(self):
        source = self.source_fixture_root / "input.txt"
    ...
    def test_html_extract_generates_canonical_outputs_without_script_or_style(self):
        source = self.source_fixture_root / "input.html"
        source.write_text(
    ...
    def test_epub_extract_generates_canonical_outputs_metadata_and_images(self):
        source = self.source_fixture_root / "input.epub"
    ...
    def test_invalid_format_fails_with_clear_error(self):
        source = self.source_fixture_root / "input.pdf"
    ...
    def test_contract_keys_are_uniform_for_all_formats(self):
        txt = self.source_fixture_root / "contract.txt"
        html = self.source_fixture_root / "contract.html"
        epub = self.source_fixture_root / "contract.epub"
    ...
    def repo_path(self, path_value: str) -> Path:
        return self.test_project_root / path_value
