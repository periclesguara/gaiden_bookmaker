...
def prepare_chapter_job(
    *,
    ...
    replacing_legacy = bool(existing and existing.schema_version != JOB_SCHEMA_V2)
    legacy_detail: dict[str, object] = {}
    if replacing_legacy:
        if not replace_legacy:
            raise ValueError(
                "Já existe um job v1 com contrato monolítico; confirme explicitamente a substituição pelo split v2."
            )
        if existing.imported_at or existing.return_sha256 or existing.status == ManualTranslationJob.STATUS_IMPORTED:
            raise ValueError(
                "O job v1 já possui retorno importado e não pode ser convertido automaticamente."
            )
        legacy_detail = {
            "previous_schema": existing.schema_version,
            "previous_status": existing.status,
            "previous_drive_path": existing.drive_path,
        }
        force = True
    resplittable = {
    ...
    if existing and not replacing_legacy and existing.status not in resplittable:
    ...
    defaults = {
        ...
        # V2 uses an isolated subfolder so a historical monolithic v1 input can
        # never be mistaken for the active chapter contract.
        "drive_path": f"{manual_translation.drive_job_path(edition.work.code, target_language)}/v2",
    ...
    with transaction.atomic():
        ...
        _record_event(
            ...
            operation=(
                "LEGACY_V1_REPLACED"
                if replacing_legacy
                else ("SPLIT_CREATED" if created else "SPLIT_RECREATED")
            ),
            previous_status=previous_status,
            ...
            detail={
                ...
                **legacy_detail,
                "new_drive_path": job.drive_path,
            },
        )

    expected_input_filenames = {unit.input_filename for unit in job.units.all()}
    local_input_dir = _job_root(job) / "input" / "chapters"
    if force and local_input_dir.is_dir():
        for stale_path in local_input_dir.iterdir():
            if (
                stale_path.is_file()
                and SAFE_FILE_RE.fullmatch(stale_path.name)
                and stale_path.name not in expected_input_filenames
            ):
                stale_path.unlink()
...
def export_chapter_job(
    ...
    *,
    ...
    published = 0
    try:
        for directory in ("", "input", "input/chapters", "return", "return/chapters"):
            active_gateway.ensure_directory(directory)
        ordered_units = list(job.units.order_by("sequence"))
        unit_payloads = {
            f"input/chapters/{unit.input_filename}": _source_unit_bytes(job, unit)
            for unit in ordered_units
        }
        contract_payloads = {
            "input/style-contract.json": _json_bytes(_style_contract(job)),
            "input/translation-job.json": _json_bytes(_job_contract(job)),
            ...
            "return/translation-return.template.json": _json_bytes(_return_manifest_template(job)),
        }
        publish_tree = getattr(active_gateway, "publish_tree", None)
        if callable(publish_tree):
            remote_rows = publish_tree({**unit_payloads, **contract_payloads})
            published = len(unit_payloads) + len(contract_payloads)
        else:
            remote_rows = {}
            for relative_path, data in unit_payloads.items():
                if _publish_immutable(active_gateway, relative_path, data):
                    published += 1
                remote_rows[relative_path] = active_gateway.stat(relative_path) or {}
        for unit in ordered_units:
            relative_path = f"input/chapters/{unit.input_filename}"
            stat = remote_rows.get(relative_path) or {}
        ...
        if not callable(publish_tree):
            for relative_path, data in contract_payloads.items():
                if _publish_immutable(active_gateway, relative_path, data):
                    published += 1
...
def import_consolidated_translation(
    job: ManualTranslationJob,
    data: bytes,
    *,
    source_label: str,
) -> dict[str, object]:
    """Register one operator-supplied, consolidated manuscript as final.

    This is the deliberately simple post-Drive contract: the Drive may still
    transport chapter files, but Step 4 receives one consolidated TXT/MD from
    the operator and does not perform per-unit return validation or a merge.
    Only file-integrity checks required to store an artifact safely remain.
    """
    if job.schema_version != JOB_SCHEMA_V2:
        raise ValueError("A importação consolidada é exclusiva de jobs v2 por capítulos.")
    if not data.strip():
        raise ValueError("O manuscrito consolidado está vazio.")
    try:
        data.decode("utf-8", errors="strict")
    except UnicodeDecodeError as exc:
        raise ValueError("O manuscrito consolidado deve estar em UTF-8.") from exc
    if job.target_edition is None:
        raise ValueError("O job não possui edição de destino para importação.")

    digest = _sha256(data)
    final_dir = storage.translated_dir(job.edition.work.code, job.target_language)
    final_path = final_dir / job.expected_return_name
    if final_path.exists() and final_path.read_bytes() != data:
        raise ValueError("Já existe um manuscrito consolidado diferente para este job.")
    _write_atomic(final_path, data)
    _write_atomic(paths.merge_translate_path(job.target_edition), data)
    artifact = _final_artifact(job, final_path, digest, len(data))
    report = {
        "schema": "gaiden_consolidated_translation_import_v1",
        "source_label": source_label,
        "final_sha256": digest,
        "size_bytes": len(data),
        "per_unit_validation": "not_requested",
        "merge": "not_requested",
    }
    _write_atomic(
        _job_root(job) / "consolidated-import.json",
        _json_bytes(report),
        allow_replace=True,
    )
    with transaction.atomic():
        ...
        _record_event(
            job,
            operation="CONSOLIDATED_IMPORT_COMPLETED",
            previous_status=previous,
            new_status=job.status,
            detail={"final_sha256": digest, "source_label": source_label},
        )
        pipeline_state, _ = EditionPipeline.objects.get_or_create(edition=job.target_edition)
        ...
        pipeline_state.last_log = f"{timezone.now().isoformat()} :: CONSOLIDATED_TRANSLATION_IMPORTED :: sha256={digest}"
        pipeline_state.save(
            update_fields=["current_stage", "translation_language", "translated_at", "last_log"]
        )
    return {"status": "COMPLETED", "path": str(final_path), "sha256": digest}
