...
SOURCE_MD = BUILD_DIR / "seneca_dialogues_v12_SOURCE.md"
OUTPUT_EPUB = BUILD_DIR / "seneca_dialogues_v12.epub"
REPORT_PATH = BUILD_DIR / "seneca_dialogues_v12_epub_report.json"
DOWNLOAD_DIR = Path("/home/periclesguara/Downloads/Dialogues\nSeneca")
DOWNLOAD_EPUB = DOWNLOAD_DIR / "seneca_dialogues_v12.epub"
DOWNLOAD_SOURCE = DOWNLOAD_DIR / "seneca_dialogues_v12_SOURCE.md"
DOWNLOAD_REPORT = DOWNLOAD_DIR / "seneca_dialogues_v12_epub_report.json"
DOWNLOAD_CANONICAL_MD = DOWNLOAD_DIR / "dialogues_seneca_v12_meditations_style_aphorisms.md"
...
def run_pandoc() -> dict[str, object]:
    ...
    css_path = BUILD_DIR / "epub_v12.css"
    css_path.write_text(
        "\n".join(
            [
                "body { margin: 0 4%; }",
...
def split_chapter_title_and_body_files(epub_path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="seneca_v12_split_") as tmp:
        workdir = Path(tmp)
...
def rewrite_glossary_links(epub_path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="seneca_v12_epub_") as tmp:
        workdir = Path(tmp)
        with zipfile.ZipFile(epub_path) as zf:
            zf.extractall(workdir)
...
def validate_epub(epub_path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="seneca_v12_validate_") as tmp:
        workdir = Path(tmp)
        with zipfile.ZipFile(epub_path) as zf:
