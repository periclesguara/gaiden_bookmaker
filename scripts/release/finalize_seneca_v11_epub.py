...
SOURCE_MD = BUILD_DIR / "seneca_dialogues_v11_SOURCE.md"
OUTPUT_EPUB = BUILD_DIR / "seneca_dialogues_v11.epub"
REPORT_PATH = BUILD_DIR / "seneca_dialogues_v11_epub_report.json"
DOWNLOAD_DIR = Path("/home/periclesguara/Downloads/Dialogues\nSeneca")
DOWNLOAD_EPUB = DOWNLOAD_DIR / "seneca_dialogues_v11.epub"
DOWNLOAD_SOURCE = DOWNLOAD_DIR / "seneca_dialogues_v11_SOURCE.md"
DOWNLOAD_REPORT = DOWNLOAD_DIR / "seneca_dialogues_v11_epub_report.json"
...
def run_pandoc() -> dict[str, object]:
    ...
    css_path = BUILD_DIR / "epub_v11.css"
    css_path.write_text(
        "\n".join(
            [
                "body { margin: 0 4%; }",
                ...
                "section.chapter-text-body { page-break-before: always; break-before: page; }",
                ...
                ".subchapter-block { page-break-inside: avoid; break-inside: avoid; }",
                ".subchapter-number { text-align: center; font-weight: bold; margin-top: 1.5em; margin-bottom: 1em; }",
                ".aphorism-inline-number { font-weight: bold; }",
...
def split_chapter_title_and_body_files(epub_path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="seneca_v11_split_") as tmp:
        workdir = Path(tmp)
        ...
        for path in sorted(text_dir.glob("*.xhtml")):
            ...
            if '<section id="chapter-' not in text or "<h2>Chapter " not in text:
                continue
            ...
            if not body_match or not section_match:
                continue

            prefix, _body_content, suffix = body_match.groups()
            title_section, chapter_id, body_fragment, _section_close = section_match.groups()
            ...
            body_fragment = wrap_xhtml_subchapter_pairs(body_fragment)
            ...
            body_text = (
                f'{body_prefix}\n<section id="{chapter_id}-body" class="chapter-text-body">\n'
                f"{body_fragment}\n</section>\n{suffix}"
            )
        ...
        return {
            "chapters_split": len(split_files),
            "split_files": split_files,
            "epub_sha256_after_split": sha256_file(epub_path),
        }


def wrap_xhtml_subchapter_pairs(fragment: str) -> str:
    pattern = re.compile(
        r'(?s)(<p class="subchapter-number">\s*\d+\s*</p>\s*)(<p>\s*<span class="aphorism-inline-number">\d+\.</span>.*?</p>)'
    )

    def repl(match: re.Match[str]) -> str:
        return '<div class="subchapter-block">\n' + match.group(1) + match.group(2) + "\n</div>"
...
def validate_epub(epub_path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="seneca_v05_validate_") as tmp:
        ...
        for path in xhtml_paths:
            ...
            aphorism_markers += text.count('class="subchapter-number"')
...
def main() -> int:
    ...
    split_report = split_chapter_title_and_body_files(OUTPUT_EPUB)
    ...
    report = {
        ...
        "chapter_title_body_split": split_report,
